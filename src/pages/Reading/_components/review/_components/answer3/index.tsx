...
const Answer3 = ({ user, correct, questions }: Answer3Props) => {
  ...
  const getCorrectQuestion = (slotIndex: number) =>
    questions.find((q) => q.reading_part_3_id === correct[slotIndex]);

  const totalCorrect = allSlotIndexes.reduce((sum, idx) => {
    const answer = slotAnswers[idx];
    return answer?.reading_part_3_id === correct[idx] ? sum + 1 : sum;
  }, 0);
  return (
    <section>
      <div className="flex items-center gap-4 mb-4">
        <h3 className="text-xl font-semibold">Question 3 of 5</h3>
        ...
        <div className="flex-1">
          ...
          <div className="space-y-2">
            {allSlotIndexes.map((idx) => {
              const answer = slotAnswers[idx];
              const isCorrect = answer?.reading_part_3_id === correct[idx];
